...
export default function Principal() {
  const [sapatosMarca, setSapatosMarca] = useState<sapatosMarcaI[]>([]);
  ...
  useEffect(() => {
    ...
    async function getDadosGrafico() {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_URL_API}/dashboard/sapatosMarca`
      );
      const dados = await response.json();
  ...
  const data: (["Marca", "NºTenis", {role: string}] | DataRow)[] = [
    ["Marca", "NºTenis", {role: "style"}],
  ];
  ...
  sapatosMarca.forEach((tenis, index) => {
    data.push([tenis.marca, tenis.num, cores[index % 10]]);
  });
    ...
    <div className="container mt-24">
      ...
      <div className="w-2/3 flex justify-between mx-auto mb-5">
        ...
        <div className="border-red-600 border rounded p-6 w-1/3 me-3">
          ...
          <p className="font-bold mt-2 text-center">Nº Tênis</p>
        </div>
