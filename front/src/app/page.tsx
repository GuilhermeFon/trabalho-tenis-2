...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function buscaDados() {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_URL_API}/sapatos`
      );
      const dados = await response.json();
      setTenis(dados);
    }
  ...
  const listaTenis = tenis.map(
    (
      tenis // Alterado para listaTenis
    ) => (
  ...
  return (
    <main>
      ...
      <section className="max-w-screen-xl mx-auto">
        <h1 className="mb-5 mt-2 text-3xl font-extrabold leading-none tracking-tight text-gray-900 md:text-4xl lg:text-5xl dark:text-black">
          Tênis{" "}
          <span className="underline underline-offset-3 decoration-8 decoration-blue-900 dark:decoration-blue-400">
            em destaque
          </span>
        </h1>
