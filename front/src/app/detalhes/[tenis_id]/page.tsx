...
export default function Detalhes() {
  ...
  useEffect(() => {
    async function buscaDados() {
      const response = await fetch(`${process.env.NEXT_PUBLIC_URL_API}/sapatos/${params.tenis_id}`) // Chamada para buscar dados do tênis
    ...
    async function buscaFotos() {
      const response = await fetch(`${process.env.NEXT_PUBLIC_URL_API}/fotos/${params.tenis_id}`) // Chamada para buscar fotos do tênis
      const dados = await response.json()
  ...
  const listaFotos = fotos.map(foto => (
    <div key={foto.id}> {/* Adicionado key para o div */}
  ...
  async function enviaProposta(data: Inputs) {
    const response = await fetch(`${process.env.NEXT_PUBLIC_URL_API}/propostas`, {
      ...
      body: JSON.stringify({
        ...
        tenisId: Number(params.tenis_id), // ID do tênis
        descricao: data.descricao // Descrição da proposta
      })
    })
    ...
    <>
      <section className="flex mt-6 mx-auto flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-5xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
        <img className="object-cover w-full rounded-t-lg h-96 md:h-2/4 md:w-2/4 md:rounded-none md:rounded-s-lg"
          src={tenis?.foto} alt="Foto do Tênis" /> {/* Exibição da foto do tênis */}
        <div className="flex flex-col justify-between p-4 leading-normal">
          <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
            {tenis?.marca?.nome} {tenis?.modelo} {/* Exibição da marca e modelo do tênis */}
          </h5>
          <h5 className="mb-2 text-xl tracking-tight text-gray-900 dark:text-white">
            Preço R$: {Number(tenis?.preco) // Exibição do preço do tênis
              .toLocaleString("pt-br", { minimumFractionDigits: 2 })}
          </h5>

          {cliente.id ?
            <>
              <h3 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">Gostou deste Tênis? Faça uma Proposta!</h3> 
            ...
            :
            <h3 className="text-xl font-bold tracking-tight text-orange-700 dark:text-white">** Faça login para fazer a proposta deste tênis</h3> 
          }
        </div>
      </section>
